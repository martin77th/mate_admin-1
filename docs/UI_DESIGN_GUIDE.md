...
- 생성 화면의 참석자 초대 목록이 존재하면, 생성 성공 직후 `PUT /api/meeting/v1/meetings/{meeting_id}/members`를 `method=set`으로 호출해 참가자를 반영합니다.
...
- 현재 진행 미팅의 `참석` 버튼은 `booked`와 `held` 상태에서만 노출합니다.
- 회의명 클릭 시 상세 페이지(`/meetings/{meeting_id}`)로 이동합니다.
- 상세 페이지는 등록 화면과 동일한 스타일의 읽기 전용 화면이며, `booked` 상태일 때만 하단 `수정` 버튼을 노출합니다.
- 상세 페이지의 `수정` 버튼 또는 목록의 `수정` 버튼은 수정 페이지(`/meetings/{meeting_id}/edit`)로 이동합니다.
- 지난 미팅 이력에서는 `수정/삭제` 액션을 제공하지 않습니다.
- 목록 우측 `관리` 컬럼의 `참석/수정/삭제` 액션은 현재 진행 미팅에서만 제공합니다.
